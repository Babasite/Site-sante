...
def construire_alias_webp():
    ...
    for dossier_images in DOSSIERS_IMAGES:
        ...
        for webp in dossier_images.rglob("*.webp"):
            ...
            alias.add(
                chemin_absolu_sans_extension.as_posix()
            )

            # Chemin relatif à la racine du dossier static.
            try:
                alias.add(
            ...
            # Nom simple, uniquement s'il n'existe pas en plusieurs exemplaires.
...
def remplacer_dans_fichier(fichier, alias_webp):
    ...
    def remplacer(correspondance):
        ...
        if chemin_original.lower().startswith(
            ("http://", "https://")
        ):
        ...
        # Vérifie différentes formes possibles du chemin.
        candidats = {
            chemin_normalise,

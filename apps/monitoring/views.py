...
def veille(request):
    ...
    if request.method == "POST":
        if not request.user.is_authenticated or not request.user.is_staff:
            messages.error(request, "Connectez-vous comme rÃ©dacteur pour lancer une veille.")
        ...
        try:
            ...
            with transaction.atomic():
                for ordre, article in enumerate(resultats, start=1):
                    ArticleVeille.objects.create(
                        ...
                        preuve=article.get("preuve", "Non dÃ©terminÃ©"),
            ...
            return render(
                ...
                "accueil/veille.html",
                {
                    ...
                    "erreur": f"La veille nâ€™a pas pu Ãªtre terminÃ©e : {erreur}",
                },
                status=500,
            )
...
def redacteur_connexion(request):
    ...
    if request.method == "POST":
        ...
        messages.error(request, "Identifiant incorrect ou compte non autorisÃ©.")
...
def modifier_resume(request, veille_id):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Le rÃ©sumÃ© a Ã©tÃ© enregistrÃ©.")
...
def modifier_traduction(request, article_id):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "La traduction a Ã©tÃ© enregistrÃ©e.")
